...
router.post('/verify', [
  ...
  try {
    ...
    if (license.status === 'suspended') {
      return res.status(400).json({ valid: false, message: 'License has been suspended.' });
    }

    // If already used and bound to a different device, reject
    if (license.status === 'used' && license.deviceId && deviceId && license.deviceId !== deviceId) {
    ...
    res.status(500).json({ valid: false, message: 'License verification failed.' });
  }
});
...
router.post('/periodic-check', protect, async (req, res) => {
  try {
    ...
    if (!license) {
      return res.status(403).json({ allowed: false, message: 'License not found.' });
    }

    if (new Date() > license.expiresAt) {
      return res.status(403).json({ allowed: false, message: 'License expired.' });
    }

    if (license.status === 'suspended') {
      return res.status(403).json({ allowed: false, message: 'License suspended.' });
    }

    // Device binding check
    if (deviceId && license.deviceId && license.deviceId !== deviceId) {
      return res.status(403).json({ allowed: false, message: 'Device mismatch. License bound to another device.' });
    }
...
router.post('/create', protect, admin, [
  body('type').optional().isIn(['trial', 'standard', 'premium']).withMessage('Invalid license type'),
  body('expiresInDays').optional().isInt({ min: 1 }).withMessage('Expires in days must be positive'),
  validate,
], async (req, res) => {
  try {
    ...
    if (attempts >= maxAttempts) {
      return res.status(500).json({ message: 'Failed to generate unique license key' });
    }
