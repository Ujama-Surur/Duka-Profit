...
const defaultForm = { productName: '', barcode: '', costPrice: '', sellingPrice: '', quantity: '', expirationDate: '', lowStockThreshold: '10', category: 'other' };
...
export default function Products() {
    ...
    try {
      ...
      if (data?.found && data?.product) {
        ...
        setForm((prev) => ({
          ...
          lowStockThreshold: product.lowStockThreshold?.toString() || prev.lowStockThreshold,
          category: product.category || prev.category,
          expirationDate: product.expirationDate ? new Date(product.expirationDate).toISOString().split('T')[0] : prev.expirationDate,
        }));
        toast.success('Product found. Form auto-filled.');
      } else {
        toast('Product not found. Enter details and save.', { icon: 'ℹ️' });
      }
    } catch (err) {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (editItem) {
        ...
        setProducts(prev => prev.map(p => p._id === editItem._id ? data : p));
  ...
  const margin = (cost, sell) => sell > 0 ? (((sell - cost) / sell) * 100).toFixed(1) : 0;
  ...
  return (
    <div className={styles.page}>
      <div className="page-header">
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:12}}>
          <div>
            ...
            <p className="page-subtitle">{products.length} products in your library</p>
          </div>
      ...
      {loading ? (
        ...
        <div className="card">
          <div className="empty-state">
            ...
            <p style={{color:'var(--text-muted)',fontSize:14}}>{t('addFirstProduct')}</p>
        ...
        <div className={styles.grid}>
          {filtered.map((product, i) => (
            <div key={product._id} className={`${styles.productCard} animate-fade-in`} style={{animationDelay:`${i*50}ms`}}>
              <div className={styles.productHeader}>
                <div className={styles.productEmoji}>{categoryEmoji(product.category)}</div>
                <div className={styles.productActions}>
                  <button className="btn btn-ghost btn-icon btn-sm" onClick={() => openEdit(product)} title="Edit">✏️</button>
                  <button className="btn btn-ghost btn-icon btn-sm" onClick={() => setConfirmDelete(product)} title="Delete">🗑️</button>
                </div>
              </div>
              ...
              <div className={styles.priceRow}>
                <div className={styles.priceItem}>
                  ...
                  <span className={styles.priceValue} style={{color:'var(--text-muted)'}}>
                ...
                <div className={styles.priceItem}>
                  ...
                  <span className={styles.priceValue}>{formatCurrency(product.sellingPrice)}</span>
                </div>
              </div>

              <div className={styles.profitRow}>
                <div>
                  <p style={{fontSize:12,color:'var(--text-muted)',fontWeight:600}}>PROFIT/UNIT</p>
                  <p className="profit-text" style={{fontSize:18,fontWeight:800}}>
                    +{formatCurrency(profit(product.costPrice, product.sellingPrice))}
                  </p>
                </div>
                <div style={{textAlign:'right'}}>
                  <p style={{fontSize:12,color:'var(--text-muted)',fontWeight:600}}>MARGIN</p>
                  <p style={{fontSize:18,fontWeight:800,color:'var(--green-primary)'}}>
              ...
              <div style={{marginTop:12,paddingTop:12,borderTop:'1px solid var(--border)'}}>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,fontSize:13}}>
                  <div>
                    ...
                    <span style={{marginLeft:4, color: product.stock <= product.lowStockThreshold ? 'var(--red)' : 'var(--text)'}}>
                      {product.stock || 0}
                    </span>
                    {product.stock <= product.lowStockThreshold && (
                      <span style={{marginLeft:4, color:'var(--red)', fontSize: '11px'}}>⚠️ Low</span>
                    )}
                  </div>
                  <div>
      ...
      {/* Add/Edit Modal */}
      {showModal && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowModal(false)} role="dialog" aria-modal="true" aria-labelledby="modal-title">
          <div className="modal" role="document" tabIndex="-1">
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:24}}>
              ...
              <button className="btn btn-ghost btn-icon" onClick={() => setShowModal(false)} aria-label="Close modal" type="button">❌</button>
            </div>
            ...
            <form onSubmit={handleSave} style={{display:'flex',flexDirection:'column',gap:18}}>
              {!editItem && (
                <div style={{display:'flex',gap:10,alignItems:'center',flexWrap:'wrap'}}>
                  ...
                  {lookupLoading && <span style={{fontSize:13,color:'var(--text-muted)'}}>Looking up product...</span>}
                </div>
              )}
              ...
              <div className="form-group">
                ...
                />
                {errors.productName && <span id="product-name-error" className="form-error" role="alert">{errors.productName}</span>}
              </div>
              ...
              <div className="form-group">
                ...
                />
                {errors.barcode && <span id="barcode-error" className="form-error" role="alert">{errors.barcode}</span>}
              </div>
              ...
              <div className="form-group">
                ...
                >
                  {CATEGORIES.map(c => (
                    <option key={c} value={c}>{categoryEmoji(c)} {t(c)}</option>
                  ))}
                </select>
              </div>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
                ...
                <div className="form-group">
                  <label className="form-label">{t('sellingPrice')} (RWF)</label>
                  <input
                ...
                {form.category === 'food' && (
                  <div className="form-group">
                    <label className="form-label">Expiration Date <span style={{color:'var(--red)'}}>*</span></label>
                    <input
                ...
                <div className="form-group">
                  ...
                  />
                  <span style={{fontSize:12,color:'var(--text-muted)'}}>Alert when stock reaches this level</span>
                </div>
              </div>

              {/* Live profit preview */}
              {form.costPrice && form.sellingPrice && Number(form.sellingPrice) > Number(form.costPrice) && (
                <div style={{padding:'14px 16px',background:'var(--green-50)',border:'1px solid var(--green-200)',borderRadius:'var(--radius-md)',display:'flex',alignItems:'center',gap:10}}>
                  <span style={{fontSize:20}}>💰</span>
                  <div>
                    <p style={{fontSize:12,color:'var(--green-dark)',fontWeight:600}}>PROFIT PER UNIT</p>
                    <p className="profit-text" style={{fontSize:20}}>
                      +{formatCurrency(form.sellingPrice - form.costPrice)}
                    </p>
                  </div>
                  <div style={{marginLeft:'auto',textAlign:'right'}}>
                    <p style={{fontSize:12,color:'var(--green-dark)',fontWeight:600}}>MARGIN</p>
                    <p className="profit-text" style={{fontSize:20}}>
          ...
          <div className="modal" style={{maxWidth:400}}>
            <div style={{textAlign:'center',marginBottom:20}}>
              ...
              <p style={{color:'var(--text-muted)',marginTop:8}}>
                Are you sure you want to delete <strong>{confirmDelete.productName}</strong>?
                This cannot be undone.
              </p>
            </div>
            <div style={{display:'flex',gap:12}}>
              <button className="btn btn-ghost btn-full" onClick={() => setConfirmDelete(null)}>{t('cancel')}</button>
              <button className="btn btn-danger btn-full" onClick={() => handleDelete(confirmDelete._id)}>
                {t('delete')}
              </button>
            </div>
          </div>
